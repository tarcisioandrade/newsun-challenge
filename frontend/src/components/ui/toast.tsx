...
export const Toast = () => {
  ...
  return (
    config.isVisible && (
      <div className="animate-toast-show absolute left-4 right-4 z-[100] rounded bg-slate-50 p-4 sm:left-1/2 sm:w-fit sm:translate-x-[-50%]">
        <div className="flex items-center gap-2">
          {props.type === "error" ? (
            <MailWarning size={20} color="#ef4444" />
          ) : (
            <MailCheck size={20} color="#22c55e" />
          )}
          <p>{props.title}</p>
        </div>
